...
def _population_log_likelihood(ll, amm, idx = None, joint = False):
    ...
    alpha_factor = 1.
    comp_idx = idx + amm.augment
    # Indices of the parameters (needed if joint)
    start = 0
    finish = len(amm.par_bounds[idx])
    if joint:
        start  += np.sum([len(amm.par_bounds[i]) for i in range(idx)])
        finish += start
    if amm.shared_par_bounds is not None:
        start_shared = -len(amm.shared_par_bounds)
        if not np.all([amm.shared_par_bounds[idx][i][0] < ll[start_shared+i] < amm.shared_par_bounds[idx][i][1] for i in range(len(ll[start_shared:]))]):
            return -np.inf
    else:
        start_shared = len(ll)
    # Check bounds
...
def _joint_population_log_likelihood(ll, amm):
    ...
    for idx in range(len(amm.par_models)):
        logL += _population_log_likelihood(ll, am, idx, joint = True)
